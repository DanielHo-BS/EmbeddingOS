...
int task_start_time[64] = { 0 };  /// �bOSTimeDly()��task�]��ready�ɬ���
            ...
            if (ptcb->OSTCBDly != 0u) {                    /* No, Delayed or waiting for event with TO     */
                ...
                if (ptcb->OSTCBDly == 0u) {                /* Check for timeout                            */
                    ...
                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* Is task suspended?       */
                        ...
                        /// PA#1
                        task_start_time[ptcb->OSTCBPrio] = OSTimeGet();
                        /// PA#1
...
static  void  OS_SchedNew (void)
{
    ...
    if (OSPrioCur == OSPrioHighRdy)
    {
        if (OSTCBCur->self_continue == 1)
        {
            ...
            /// task_preempt_count[OSPrioHighRdy]++;  /// same task

            task_start_time[OSTCBCur->OSTCBPrio] = OSTimeGet();
        }
    }
    /// =======================================Special Case completion=======================================
    ...
    else if ((OSPrioCur != OSPrioHighRdy && OSTimeGet() != 0))
    {
        ...
        else
        {
            ...
            /// =======================================Completion=======================================
            if ((OSRdyTbl[OSPrioCur >> 3] & OSMapTbl[OSPrioCur & 0x07]) == 0)  
            {
                printf("  Completion\t");
                fprintf(Output_fp, "  Completion\t");
                /// CurrentTask
            ...
            else  
            {
                ...
                if (OSTCBCur->remain_exe_time == 1)
                {
                    ...
                    fprintf(Output_fp, "%7d\t\t", task_preempt_count[OSPrioCur]);
                    /// PreemptionTime
                    printf("%4d/t", task_preempt_time_acc[OSPrioCur]);
                    fprintf(Output_fp, "%4d/t", task_preempt_time_acc[OSPrioCur]);
                    // DelayTime: Period - ResponseTime
                    printf("%11d\n", (TaskParameter[OSPrioCur - 1].TaskPeriodic - (OSTimeGet() - task_start_time[OSPrioCur])));
                    fprintf(Output_fp, "%11d\n", (TaskParameter[OSPrioCur - 1].TaskPeriodic - (OSTimeGet() - task_start_time[OSPrioCur])));
