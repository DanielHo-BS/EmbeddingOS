                     ...
                     INT8U    prio)
{
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        err = OS_TCBInit(prio, psp, (OS_STK *)0, 0u, 0u, (void *)0, 0u, 0, 255, 255);
        if (err == OS_ERR_NONE) {
...
INT8U  OSTaskCreateExt (void   (*task)(void *p_arg),
                        ...
                        void    *pext,
                        INT16U   opt,
                        INT8U arrive_time, INT8U exe_time, INT8U period)
{
    OS_STK     *psp;
    INT8U       err;
#if OS_CRITICAL_METHOD == 3u                 /* Allocate storage for CPU status register               */
    ...
    if (OSTCBPrioTbl[prio] == (OS_TCB *)0) { /* Make sure task doesn't already exist at this priority  */
        ...
        err = OS_TCBInit(prio, psp, pbos, id, stk_size, pext, opt, arrive_time, exe_time, period);  /// printf('Task ... created...') here
        if (err == OS_ERR_NONE) {
            OS_TRACE_TASK_CREATE(OSTCBPrioTbl[prio]);
