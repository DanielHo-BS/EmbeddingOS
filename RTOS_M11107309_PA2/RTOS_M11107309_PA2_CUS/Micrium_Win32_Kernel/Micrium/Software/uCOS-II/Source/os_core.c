...
/// PA#1�A�Ω�T�{OSRdyTbl�����Y��priority�O�_��1
INT8U OSMapTbl[8] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80 };

/// PA#1�A�Ω�p��U��Task��response time�B�����Qpreempt�����ơB�ɶ�(# of ContextSwitch, PreemptionTime)
int task_start_time[64] = { 0 };  /// ����task ready���ɶ��A�b�����ɥΨӭp��response time
int response_time = 0;  /// ����������task��response time
int task_preempt_count[64] = { 0 };
int task_preempt_time[64] = { 0 };
int task_preempt_time_acc[64] = { 0 };

/// PA#2�A�Ω����miss deadline��prio�A�L�h��-1
int miss_deadline_prio = -1;
int aperiodic_job_start_time[OS_MAX_TASKS] = { 0 };  /// ����job arrive���ɶ��A�b�����ɥΨӭp��response time

/// PA#2�AFor EDF�A�ݹLOSRdyTbl�@�M�A�ˬd�O�_��miss deadline�A���hretrun miss deadline��prio�A�L�hreturn-1
int check_deadline()
{
    int miss_deadline_prio = -1;
    for (int prio = 0; prio <= OS_LOWEST_PRIO; prio++)
    {
        int row = prio / 8;
        int col_mask = 0b1 << (prio % 8);

        if ((OSRdyTbl[row] & col_mask) != 0)  // �p�G�Y��task�Oready��
        {
            if (OSTCBPrioTbl[prio]->deadline <= OSTimeGet())
            {
                miss_deadline_prio = prio;
                break;
            }
        }
    }
...
int job_index_queue[OS_MAX_TASKS] = { -1 };  // ����server�n�h���檺job�A��bAperiodicJobParameter��index
int front = -1;  // ���Vqueue���̫�@��job -1����m
int tail = -1;  // ���Vqueue���̫�@��job
int current_job_id = -1;

/// PA#2�AFor CUS�A�ݹLAperiodicJobParameter�@�M�A�ˬd�O�_��aperiodic job�n�}�l
void enqueue_aperiodic_job()
{
    for (int i = 0; i < OS_MAX_TASKS; i++)
    {
        if (AperiodicJobParameter[i].TaskArriveTime == OSTimeGet())
        {
            tail++;
            job_index_queue[tail] = i;
            aperiodic_job_start_time[AperiodicJobParameter[i].TaskID] = OSTimeGet();
        }
    }
}
void dequeue_aperiodic_job()
{
    front++;
}
...
void  OSTimeTick (void)
{
    ...
    if (OSRunning == OS_TRUE) {
            ...
            if (ptcb->OSTCBDly != 0u) {                    /* No, Delayed or waiting for event with TO     */
                ...
                if (ptcb->OSTCBDly == 0u) {                /* Check for timeout                            */
                    ...
                    if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* Is task suspended?       */
                        ...
                        /// PA#1
                        // task_start_time[ptcb->OSTCBPrio] = OSTimeGet();  // �����o��job ready�ɶ��A�Ω�p��response time
                        /// PA#1
        ...
        /// PA#2�A��remain_exe_time�B�z���OSTimeTick()�B�z
        if (OSTCBCur->remain_exe_time > 0)  
        {
            OSTCBCur->remain_exe_time--;
            // printf("tick %d choose %d to --\n", OSTimeGet(), OSTCBCur->OSTCBPrio);

            if (OSTCBCur->remain_exe_time == 0)
            {
                if (OSTCBCur->OSTCBId == SERVER_ID)
                {
                    response_time = OSTimeGet() - aperiodic_job_start_time[current_job_id];
                }
                else
                {
                ...
                if (OSTCBCur->OSTCBId == SERVER_ID)
                {
                    ...
                    fprintf(Output_fp, "Aperiodic job(%d) is finished.\n", current_job_id);
                    fclose(Output_fp);
                }
                else
                {
                    task_start_time[OSTCBCur->OSTCBPrio] += OSTCBCur->period;  // �����U��job ready�ɶ��A�Ω�p��response time

                    OSTCBCur->arrive_time = OSTCBCur->arrive_time + OSTCBCur->period;  // �]�U��job�}�l�ɶ�
                    OSTCBCur->deadline = OSTCBCur->arrive_time + OSTCBCur->period;  // �]�U��job��deadline
                    OSTCBCur->remain_exe_time = OSTCBCur->exe_time;  // ���]�Ѿl����ɶ�
                }
                ...
                if (delay_ticks > 0u) {                /* 0 means no delay!                                  */
                    ...
                    OSTCBCur->OSTCBDly = delay_ticks;  /* Load ticks in TCB                                  */
                }
            }
        }
        ...
        if (miss_deadline_prio != -1)  // �bOSTimeTick()�ˬd�L
        {
            if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) != 0)
            {
                printf("can't open Output.txt");
        ...
        if (front != tail && (OSTimeGet() >= ptcb->deadline || ptcb->arrive_time == 0))  // queue�̦�job�ݰ���A�B�ثeCUS���Ŷ�(OSTimeGet() > server deadline or ��l���A)
        {
            if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* Is task suspended?       */
                OSRdyGrp |= ptcb->OSTCBBitY;             /* No,  Make ready          */
                OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
            ...
            ptcb->arrive_time = OSTimeGet();
            ptcb->deadline = OSTimeGet() + AperiodicJobParameter[job_index_queue[front + 1]].TaskExecutionTime * 100 / SERVER_SIZE;
            ...
            if (AperiodicJobParameter[job_index_queue[front + 1]].TaskArriveTime == OSTimeGet())  // arrive�ɡA��nCUS�i�H�����}�l
            {
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) != 0)
                {
                    printf("can't open Output.txt");
            ...
            dequeue_aperiodic_job();  // ��CUS�}�l����A�N�����Xqueue
        }
        else if (front != tail && OSTimeGet() < ptcb->deadline)  // queue�̦�job�ݰ���A�B�٨S��CUS deadline
        {
            for (int i = front+1; i <= tail; i++)
            {
                if (AperiodicJobParameter[job_index_queue[front + 1]].TaskArriveTime == OSTimeGet())  // �٨S�QCUS���檺job�|�@���bqueue���ݡA���u�ݦbarrive��print�@��
                {
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) != 0)
                    {
                        printf("can't open Output.txt");
                    ...
                    printf("%2d\t", OSTimeGet());
                    fprintf(Output_fp, "%2d\t", OSTimeGet());
                    printf("Aperiodic job(%d) arrives. Do nothing.\n", AperiodicJobParameter[job_index_queue[i]].TaskID);
                    fprintf(Output_fp, "Aperiodic job(%d) arrives. Do nothing.\n", AperiodicJobParameter[job_index_queue[i]].TaskID);
                    fclose(Output_fp);
                }
            }
        }
    ...
    if (OSIntNesting == 0u) {                          /* Schedule only if all ISRs done and ...       */
        if (OSLockNesting == 0u) {                     /* ... scheduler is not locked                  */
            ...
            if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy     */
                ...
                OS_TASK_SW();                          /* Perform a context switch                     */  /// OSPrioCur�b�o�̧���
            }
        }
    }
    OS_EXIT_CRITICAL();
...
/// PA#2�AFor EDF�A�ݹLOSRdyTbl�@�M�A��Xdeadline�̪�task��prio
int find_ED_prio()
{
    ...
    for (int prio = 0; prio <= OS_LOWEST_PRIO; prio++)
    {
        ...
        if ((OSRdyTbl[row] & col_mask) != 0)  // �p�G�Y��task�Oready��
        {
            ...
            if (OSTCBPrioTbl[prio]->deadline < ED)  // �p�G�o��task��deadline�O�ثe�ݨ�̤p��
            {
                ED_prio = prio;
...
static  void  OS_SchedNew (void)
{
    ...
    if (OSPrioCur != OSPrioHighRdy)
    {
        ...
        else
        {
            ...
            /// =======================================�@��Completion=======================================
            if(OSTCBPrioTbl[OSPrioCur]->remain_exe_time == OSTCBPrioTbl[OSPrioCur]->exe_time)  /// ��ܭ쥻��Task�w�����A�i�J���ݪ��A
            {
                printf("Completion\t");
                fprintf(Output_fp, "Completion\t");
                /// CurrentTask
                if (OSPrioCur == 63)  /// ��ꤣ�|�o�ͳo�ر��p
                {
                ...
                /// ResponseTime
                printf("%5d\t\t", response_time);
                fprintf(Output_fp, "%5d\t\t", response_time);
                /// # of ContextSwitch
            ...
            else  /// ��ܭ쥻��Task�|�������Apreempt
            {
                ...
                task_preempt_time[OSPrioCur] = OSTimeGet();   /// �O����ɳQpreempt     
                task_preempt_count[OSPrioCur]++;
...
INT8U  OS_TCBInit (INT8U    prio,
                   ...
                   void    *pext,
                   INT16U   opt,
                   INT8U arrive_time, INT8U exe_time, INT8U period)  /// PA#2
{
    OS_TCB    *ptcb;
#if OS_CRITICAL_METHOD == 3u                               /* Allocate storage for CPU status register */
    OS_CPU_SR  cpu_sr = 0u;
    ...
    if (ptcb != (OS_TCB *)0) {
        ...
        OSTCBInitHook(ptcb);  /// printf('Task ... created...') here
        ...
        //printf("Previous TCB pint to address %9.0p\n", ptcb->OSTCBPrev);   /// (OS_TCB *)0��pointer type�A��%p
        ...
        if (id == OS_TASK_IDLE_ID)
        {
            ...
            ptcb->period = 255;
            ptcb->deadline = 255;
            ptcb->remain_exe_time = 255;
        }
        else
        {
            ptcb->arrive_time = arrive_time;
            ptcb->exe_time = exe_time;
            ptcb->period = period;
            ptcb->deadline = arrive_time + period;
            ptcb->remain_exe_time = exe_time;
        }

        /// PA#2 �p�Garrive time == 0�A�]��ready�F�Ϥ��]�wOSTCBDly = arrive_time
        if (arrive_time == 0)
        {
