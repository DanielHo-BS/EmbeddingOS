...
int task_start_time[64] = { 0 };  /// 紀錄task ready的時間，在完成時用來計算response time
int response_time = 0;  /// 此次完成的task之response time
int task_preempt_count[64] = { 0 };
int task_preempt_time[64] = { 0 };
int task_preempt_time_acc[64] = { 0 };

/// PA#2，用於紀錄miss deadline的prio，無則為-1
int miss_deadline_prio = -1;
int aperiodic_job_start_time[OS_MAX_TASKS] = { 0 };  /// 紀錄job arrive的時間，在完成時用來計算response time
...
int job_index_queue[OS_MAX_TASKS] = { -1 };  // 紀錄server要去執行的job，其在AperiodicJobParameter的index
int front = -1;  // 指向queue的最後一個job -1的位置
int tail = -1;  // 指向queue的最後一個job
int current_job_id = -1;

/// PA#2，For CUS，看過AperiodicJobParameter一遍，檢查是否有aperiodic job要開始
    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSTCBCur->remain_exe_time > 0)  
        {
            ...
            if (OSTCBCur->remain_exe_time == 0)
            {
                ...
                {
                    task_start_time[OSTCBCur->OSTCBPrio] += OSTCBCur->period;  // 紀錄下個job ready時間，用於計算response time
        ...
        if (miss_deadline_prio != -1)  // 在OSTimeTick()檢查過
        {
            if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) != 0)
            {
                printf("can't open Output.txt");
        ...
        if (front != tail && (OSTimeGet() >= ptcb->deadline || ptcb->arrive_time == 0))  // queue裡有job待執行，且目前CUS為空閒(OSTimeGet() > server deadline or 初始狀態)
        {
            if ((ptcb->OSTCBStat & OS_STAT_SUSPEND) == OS_STAT_RDY) {  /* Is task suspended?       */
                OSRdyGrp |= ptcb->OSTCBBitY;             /* No,  Make ready          */
                OSRdyTbl[ptcb->OSTCBY] |= ptcb->OSTCBBitX;
            ...
            if (AperiodicJobParameter[job_index_queue[front + 1]].TaskArriveTime == OSTimeGet())  // arrive時，剛好CUS可以直接開始
            {
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) != 0)
                {
                    printf("can't open Output.txt");
            ...
            else  // job先等待了前一個CUS deadline，之後才開始
            {
                if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) != 0)
                {
                    printf("can't open Output.txt");
            ...
            dequeue_aperiodic_job();  // 讓CUS開始執行，就先移出queue
        }
        else if (front != tail && OSTimeGet() < ptcb->deadline)  // queue裡有job待執行，且還沒到CUS deadline
        {
            for (int i = front+1; i <= tail; i++)
            {
                if (AperiodicJobParameter[job_index_queue[front + 1]].TaskArriveTime == OSTimeGet())  // 還沒被CUS執行的job會一直在queue等待，但只需在arrive時print一次
                {
                    if ((Output_err = fopen_s(&Output_fp, "./Output.txt", "a")) != 0)
                    {
                        printf("can't open Output.txt");
